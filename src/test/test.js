...
describe('Testing the book endpoints:', () => {
	it('It should display (Welcome to this API)', done => {
		chai
			.request(app)
			.get('/')
	...
	it('It should not create a book with incomplete parameters', done => {
		const book = {
			price: '$9.99',
			description: 'test This is the awesome book'
		};
		chai
			.request(app)
			.post('/api/v1/books')
	...
	it('It should update a book', done => {
		...
		const updatedBook = {
			id: bookId,
			title: 'test Updated Awesome book',
			price: '$10.99',
			description: 'test We have updated the price'
		};
		chai
			.request(app)
